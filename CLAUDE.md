...
- **Overbids after the 2026-09-22 session (three rules, one PR)**: session 9c0a6a742dba bid +35% on Burger, +35% on Querfeld (−7%/7d) and +32% on Itakura (−19%/7d, grade C); Marco cancelled the last two by hand. (1) In `SmartBidding.calculate_ep_bid` the learned override (`BidLearner.get_ep_recommended_overbid`) replaces the stack as the *base* only — the trend factor (×0.3/0.5/0.75 for falling, ×0.6 unknown), the contested bump and the ceiling apply after it. Before, the override ran last and undid the trend cut (`stack=7.5% learned=32.3% applied=32.3%`). (2) An empty slot is not a 0.0-point starter: `scoring/decision.best_eleven_gain` (used by both `calculate_marginal_ep` and `plan_buys`) measures a candidate who fills an empty slot against `scoring/v2/adapter.cold_start_starter_ep()` — the scorer's own EP for an unknown regular starter at the position (GK 81.5 / DEF 74.9 / MID 68.4 / FW 62.4 on 2026-09-22). A grade-C body at the prior gains nothing; Burger went from +114.7 "must_have" to +46.3 "strong_upgrade". The board says "Fills an empty slot; measured against an unknown Midfielder starter (68.4)". (3) `BidCeilingPolicy.price_bands` (`OVERBID_PRICE_BANDS`, default off) caps the tier percentage from above per market-value band — a band only ever lowers a ceiling; the euro floor stays; the bidder and the gate read the same policy. `rehoboam derive-ceilings` prints, per band, our median bid against the winners' p25/p50/p75 and proposes each band's p75, refusing below 30 winners (18 on 2026-09-22: at 15m+ we bid a median +29.7%, winners paid +8.6%). The replay is inert for all three by construction (no learner, trend 0.0, its own gain and default policy): verify with `status` on the live market.
- **What it took to win, what the buy was worth (2026-09-23, migration 024)**: rival offers are never visible on Kickbase; completed transfers are. `rehoboam.transfer_premiums` prices every buy in `manager_transfers` (five seasons, 4,543 buys back to 2021-07) against the market value in force — the `mv_series` point dated Berlin time minus 22 hours, because Kickbase publishes at 22:00 (verified against our own bids' asking prices). `mv_series` starts 2025-07-29 and Kickbase keeps only 365 days, so `premium_pct` is null before that; `transfer_outcomes` adds the same manager's next sale (realised profit, days held) and the player's points over the next five played matches, back to 2021. `derive-ceilings` now reads the league's priced buys (about 1,250) with our `auction_outcomes` beside them; `transfer-study` reports by season × band and by manager (`store/transfer_study.py`). Read-only; nothing in trading reads either view yet. Apply 024 as admin before deploying.
- **The bid is read off the win curve (2026-09-23)**: `services/win_curve.WinCurve` sorts the league's winning premiums (`transfer_premiums`, ours excluded, last `BID_CURVE_LOOKBACK_DAYS`) per price band (`BID_CURVE_BANDS`); `SmartBidding.calculate_ep_bid` takes the base of the bid from it at the tier's quantile (`BID_CURVE_Q_*`: must-have p75, strong p50, solid p25, marginal the floor) instead of the typed stack (5% + tier + confidence + league + demand, or the EP-derived "learned" override). The trend factor, the contested bump, the ceiling policy and the euro floor still apply after it. A band thinner than `BID_CURVE_MIN_SAMPLE` borrows the pooled curve; a pooled curve thinner than that yields no curve and the stack (`_stack_overbid`) is the fallback — the replay has no store, so it is inert for this change by construction. `Trader._load_win_curve` runs one query per Trader and logs `win curve: N priced league buys`; a failure logs an exception and falls back. Measured 2026-09-23 on 1,397 buys: the median winner paid +8–10% in every band; p75 was +31.7% (\<5m), +21.1% (5–15m), +18.4% (15m+).
- **The cost of winning (2026-09-23)**: `services/profit_curve.ProfitCurve` reads `transfer_outcomes` (ours excluded) in 3-point premium buckets per band and finds the break-even — the first bucket with ≥ `BID_PROFIT_MIN_SAMPLE` resold buys whose median resale profit is negative (measured: about +3% above 15m, +12% at 5–15m, none under 5m). `calculate_ep_bid` caps every tier below must-have at it, after the win curve and before the trend factor; a must-have is not capped and the reasoning names the expected resale at its premium. It only ever lowers a bid: under 5m the measured premium is inflated by the player's own rise during the listing (premium is against the value at close, not at listing), so "pay more under 5m" is partly an artefact and is never acted on. Flips already bid at most `MAX_FLIP_OVERPAY_PCT` (1%), below every break-even, and are untouched. `Trader._load_curves` loads both curves in one connection and caches them per process for an hour (`_CURVE_CACHE`). `derive-ceilings` prints the break-even beside the winners' p75. `BID_PROFIT_CAP_ENABLED` turns the cap off.
- **The next update and urgency (2026-09-23)**: the auction settles after the next nightly market-value move, so `calculate_ep_bid(forecast_change_pct=…)` judges the premium against the value then — a falling forecast lowers the bid to the same premium on tomorrow's value ((1+p)(1+f)−1), a rising one changes nothing because the gate checks the ceiling against today's value (REH-99). `Trader._load_forecasts` reads `mv_forecasts` for `services/mv_forecast.next_update_day(now)` (today before 21:45 Berlin, tomorrow from 22:30, none in between), unscored rows only, cached an hour; `BID_FORECAST_ENABLED`. `urgent` (squad cannot field eleven and kickoff within `BID_CURVE_URGENCY_DAYS`=3) reads every tier `BID_CURVE_URGENCY_BUMP`=0.15 higher on the win curve, capped at p95, carried through `get_ep_recommendations` as `result["urgent"]`. Spike the same day: against the ask at listing (n=48, one week of `market_listings`) the premiums are within 4 points of the close-based ones, so the close-based `transfer_premiums` measure stands.
- **The value bid (2026-09-23)**: with a price per expected point — `store/transfer_study.eur_per_point`, the price-on-points slope behind `fair_price` (likely starters, 5m+; None below r² `BID_VALUE_MIN_R2`=0.3; 255,008 on 2026-09-23) — `calculate_ep_bid` no longer reads the win curve at the tier's quantile: `services/value_bid.optimal_premium` picks the premium maximising F(p) × (€/pt × unique gain − p × MV), where the unique gain is the marginal gain over the next-best viable candidate at the position (`scoring/decision.alternative_gains`, `trader._pair_alternatives`; carried as `result["alternative_gains"]`) plus 100 points when urgent (the empty-slot penalty). Why: without the alternative term the optimum pinned every large gain to the top of the curve (Burger +19%, a 114-point must-have +34%), the behaviour that lost money; with Schick as the runner-up Burger's unique gain is 5.3 points and the optimum is about +3%. The tier quantiles (`BID_CURVE_Q_*`) remain only as the fallback when the price per point or a positive unique gain is missing. Break-even cap, forecast, trend, contested bump, ceiling and floor still apply after it. `BID_VALUE_ENABLED` turns it off.
- **Only a fitted must-have escapes the break-even cap (2026-09-23)**: `calculate_ep_bid(data_grade=…)` exempts a must-have from the band's break-even cap only when the score's data-quality grade is "A"; a must-have scored on the position prior (grade C, or unknown) is capped like a strong upgrade and the reasoning says "must-have on data grade C, not fitted". Why: Itakura's 63.5-point swap gain was a grade-C prior against a weak player, priced at 16m of worth by the value bid and bid to the ceiling; only his falling trend saved it. The trader passes the grade at all four call sites (`_grade_of`).
- **A flip is a flip from bid to sale (2026-09-24, migration 025)**: on 2026-09-23/24 a nine-man squad with EUR 708k placed nothing twice — the profit-sell loop skips every player in the best eleven, and nine players *are* the best eleven, so nothing was ever sellable; the flip buy guard demanded that squad-plus-flip could field eleven, which nine plus one cannot, so two rising-trend flips were dropped as "unfieldable". The bot knew a buy was a flip at the bid and dropped it one step later (`tier=None`). Now `learning/tracker.FlipIntent` (target, hold days) rides `ExecutionService.buy(flip=…)` → `record_bid_placed` → `pending_bids.intent/target_pct/max_hold_days` → on a win `tracked_purchases` (same columns; `'flip'`, `'points'`, or NULL for a feed-recovered basis). `run_profit_sell_phase` reads `get_tracked_purchases(intent="flip")` first and trades a marked flip on flip rules whatever the squad looks like: sell at the trend-adjusted target (`_sell_threshold_for_trend`) or at `max_loss_pct` with no replacement required (a flip is not a starter by intent); the hold-period guard and the position minimum still apply; the hold limit is *reported*, never a trigger (Marco: hold for the target or the stop-loss). Loops 1 and 2 skip marked flips. `_flip_worsens_fieldability` makes the buy guard relative (only a squad that can field eleven today and could not afterwards is refused). Existing players are unmarked; mark one by hand with an `update` on `tracked_purchases.intent` if wanted. Still open from the same diagnosis: pairs run only at a full squad (`available_slots > 0` → skip), so Castello Jr. → El Ouahdi (+63.5 EP, +10m cash) is never attempted below 15/15.
- **Sell more often when he is not a top player (2026-09-24, migration 026)**: Marco's learning after Castello Jr. (bought 27.1m on 2026-09-01 at +35% over a 20.1m value; 15.6m and status 2 by 09-24): "sell more often if not a top player, and don't overpay". The second half is the win-curve / break-even / value-bid work; this is the first. `rehoboam.player_ranks` (a view over `player_table`, not a re-issue of it) ranks every player at his position league-wide: `avg_points_rank_pos` by points per appearance among players who have played this season (null otherwise — unknown quality is not bad quality), plus `points_rank_pos`, `ep_rank_pos`, `mv_rank_pos`, `position_size`. Why that ranking: by this week's EP Castello Jr. was 181 of 210 defenders, by total points 60, by points per appearance about 6 — the first two sell a good player because he is out this week, the third keeps him and still sells the benchwarmers. `services/rank_sell.rank_sell_reason` (pure) sells a held player who is **out for weeks** (status 1, any rank) or **outside the top `SELL_RANK_FLOOR`** (30) at his position on *both* points per appearance and expected points (`ep_rank_pos`; either inside keeps him — Burger, 44th / 7th two days after a 24.5m buy, is a hold; Møller Wolfe, 100th / 43rd, is the sale) with at least `SELL_RANK_MIN_APPEARANCES` (2) appearances, not rebounding (7d ≥ +1% defers, the loss-sell guard), and — for a member of the best eleven — only with `min_days_to_match_for_starter_swap` before kickoff (the pair guard). Status 2 ("uncertain") is not this rule. `run_profit_sell_phase` runs it after the flip and profit loops via `LeagueStore.position_ranks`; it is a points decision — not gated on `enable_profit_sells`, reads no cost basis, so it covers the I4 player without one; `SELL_RANK_ENABLED` turns it off; a failing store read sells nothing on rank. Hold-period guard and position minimum still apply. `web_players` appends `avg_points_rank_pos` and `ep_rank_pos`; the Players page can rank by the first. Apply 026 as admin before deploying (done 2026-09-24).
- **The flip search runs even when the EP pipeline has nothing (2026-09-25)**: `run_unified_trade_phase` used to print "No actionable opportunities" and return as soon as the EP candidate list (bid-able buys + pairs) was empty — *before* the profit-flip search. On 2026-09-24 20:00 and 2026-09-25 08:00 the market had nothing worth buying for points, so the phase exited and Becker (Kickbase-listed, +16.2%/14d, 163 pts/game, the flip rule's one candidate, verified read-only with the bot's own `find_profit_opportunities`) was never looked at. The early exit now sits after the flip block and fires only when both lists are empty. Diagnostic tell: the flip search always prints "Filtered … small sample size" for a spiking listing (Krattenmacher that day); its absence means the search did not run.
- `auto` command: Unified aggressive trading — single EP pipeline call, trade pairs compete with plain buys ranked by EP, matchday-aware aggressiveness (aggressive 5+d, moderate 2-4d, locked 0-1d), trend-aware profit selling, up to 10 trades/session (15 aggressive). Plain improvement buys execute through `_execute_buy` (see above).
- **The emergency fill is the last day only (2026-09-22)**: a squad that cannot field a legal eleven is still detected in every phase, but the fill — the "buy almost anything" path with the relaxed filters (EP floor 10, no target bar, no upgrade threshold, no pacing) and the leftover spent as overbid — may only spend when `services/emergency_window.emergency_fill_due` says so: `days_until_match <= Settings.emergency_fill_days` (default 1, re-tunable from `.env`), or an unknown schedule (the 2026-08-31 REH-112 state, which still fails toward fielding an eleven). Earlier than that the session logs `squad short by N … emergency fill waits` and the ordinary trading phases close the slot with players that pass the ordinary bars at ordinary bids. Both places that ask — `Trader.get_ep_recommendations` (which relaxes the filters and reports `emergency` in its result) and the session's step 3 — read the same day count: the session passes it in, so the phase and the emergency decision share one kickoff lookup. Why: on 2026-09-22 08:00 UTC, ten players and seventeen days to kickoff (the October break), the fill bought Seol at 1,951,380 — falling, unlikely to start — and Marco cancelled it by hand: "Emergency mode should only be activated 1 day before the line ups are locked in."
- **Debt until gameday (2026-09-22)**: Marco's rule is "it can go into minus until gameday, always". Every trading phase — aggressive *and* moderate — may spend `current_budget + max_debt` (60% of team value, `_compute_flip_budget`); over-budget buys still carry their sell plans. The locked window (the last two days, `days_until_match <= 1`) repays what is left: `_run_debt_recovery` runs **before** the emergency fill in every mode and instant-sells until the wallet net of open offers is ≥ 0, choosing by `services/debt_recovery.plan_debt_recovery` — profitable bench first, then profitable starters by fewest expected points, then losses (bench before starters, slumping prices last); a player at his position minimum only as the last resort, because the fill that runs right after buys the slot back and Marco does not want a squad floor ("the bot can trade below eleven if it makes sense and he fills the slots", 2026-09-22). The old step-5 forced sell (`optimize_and_execute_squad`) is untouched and still only fires early when even the whole bench could not cover the debt.
...
- `status` command: Read-only diagnostic — runs the full EP pipeline in dry-run mode so you can preview what `auto` would do. Genuinely read-only: `ExecutionService.buy` short-circuits on `dry_run` after the gate and `_execute_buy` records nothing in dry-run, so a diagnostic run neither places an offer, writes a `trade_proposals` row, nor sends the board. Its `Trades:` / `Offers:` / `Total spent:` lines describe what a live run would have done — nothing was placed.
...
- Budget-at-kickoff safety: the locked phase (0-1 days) places no new offers, `ExecutionService.buy` refuses any buy that would leave the wallet negative within `LOCKOUT_DAYS`, and `_run_debt_recovery` sells the wallet back to zero in that window (see "Debt until gameday").
...
uv run rehoboam derive-ceilings                        # Per price band: the league's buys (premium over the day's MV) vs our bids; proposes OVERBID_PRICE_BANDS (refuses < 30)
uv run rehoboam transfer-study [--season 2026/2027]    # Every buy since 2021: premium paid, points after, resale profit — by season/band and by manager
...
1. Step 2b — in the locked window only, `_run_debt_recovery` instant-sells until the wallet net of open offers is ≥ 0 (a negative budget at kickoff is zero points).
1. Steps 3-7 — emergency fill / locked or lineup-only exit / sell phase / squad optimization / unified trade phase. Bids placed via `api.buy_player`, sells via `api.sell_player_instant` or `api.sell_player`.
