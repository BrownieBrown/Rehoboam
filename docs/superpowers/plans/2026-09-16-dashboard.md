...
- **Every page is a server component, rendered per request.** `requireSession()` reads cookies, which makes the route dynamic in Next 15: there is no route cache, and `export const revalidate = 300` does nothing, so no page exports it (the task snippets below that still show it predate this correction). No client-side data fetching, no SWR, no React Query.
