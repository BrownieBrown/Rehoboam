-- Fix a regression from the mv-forecast nightly pass (2026-09-17): the
-- nightly re-reads every player's status right after the ~22:00 update,
-- overwriting `player_status_daily`'s (player_id, day) row with the
-- post-update value. `trend_24h_pct` used to compare that row against the
-- row for `day <= current_date - 1`, so once the nightly landed the two
-- would be the *same* update for most of the day and the column read 0.00 %.
--
-- `trend_24h_pct` now reads Kickbase's own last move off the newest status
-- reading directly -- the `mv_change` column already carries it (`tfhmvt`,
-- migration 008), it needs no second row, and it is right whichever side of
-- the update the reading was taken on. A null `mv_change` (rows written
-- before 008, or no status row at all) gives a null trend, not a zero --
-- arithmetic on a null operand is null.
--
-- `trend_7d_pct` still needs a week-old reference point, but the daily
-- status rows now shift by one update depending on when in the day they
-- were read, so `mv_series` (weekly per player, unaffected by the nightly)
-- is preferred over the status rows instead of the other way round.
--
-- Views only append/change columns in place, so the running site and bot
-- keep working; `web_players`/`web_market` are copied unchanged from
-- 009/010 -- they are downstream of `player_table`, not touched directly.
