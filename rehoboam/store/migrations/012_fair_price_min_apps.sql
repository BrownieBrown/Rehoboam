-- A fair price needs more than one match behind it (2026-09-17).
-- The price reads a player's season average back through his position's
-- price-to-points line, so with one or two appearances a single big score
-- moves it by millions: measured on the live store, players with one
-- appearance sat a mean 8.1 m away from their market value. Below three
-- appearances the column now says nothing instead of something false.
-- Only `player_table` changes; its column list is identical, so the views
-- that read it by name are untouched.
