...
def test_migrate_refreshes_the_bot_role_grants_on_new_tables(store_dsn, tmp_path, monkeypatch):
    with connect(store_dsn) as conn:
        ...
        conn.commit()
        # store_dsn already has versions 1-18 applied from the real
        # migrations dir; use 019 so this simulated file is genuinely new.
        (tmp_path / "019_simulated.sql").write_text(
...
def test_applying_a_new_file_under_the_bot_role_fails_clearly(store_dsn, tmp_path, monkeypatch):
    with connect(store_dsn) as conn:
        ...
        # store_dsn already has versions 1-18 applied from the real
        # migrations dir; use 019 so this simulated file is genuinely new.
        (tmp_path / "019_simulated.sql").write_text("create table rehoboam.t_new (x integer);\n")
        monkeypatch.setattr("rehoboam.store.migrate.MIGRATIONS", tmp_path)
        conn.execute(f"set role {ROLE}")
        conn.commit()
        try:
